...
class Custom_Dataset(torch.utils.data.Dataset):
    def __init__(self, data_dir, annot_file, transforms=None):
        self.data_dir = data_dir
        self.transforms = transforms
        self.annot_file = annot_file
        # load the annotations file, it also contain information of image names
        # load annotations
        annotations1 = json.load(open(os.path.join(data_dir, annot_file)))
        # print(annotations1)
...
#             xmax = np.max(self.annotations[idx]["regions"][i]["shape_attributes"]["all_points_x"])
            xmax = self.annotations[idx]["regions"][i]["shape_attributes"]["x"] + self.annotations[idx]["regions"][i]["shape_attributes"]["width"]
...
#             ymax = np.max(self.annotations[idx]["regions"][i]["shape_attributes"]["all_points_y"])
            ymax = self.annotations[idx]["regions"][i]["shape_attributes"]["y"] + self.annotations[idx]["regions"][i]["shape_attributes"]["height"]
